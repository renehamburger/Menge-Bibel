...
import { bibleBooks, BibleBook } from './menge-bible-online-chapters';
...
(async function () {
  for (const type in bibleBooks) {
    for (const book of bibleBooks[type]) {
      const currentPath = `${ROOT_PATH}/Bibel/${type}/${book.bookName}`;
      fse.ensureDirSync(currentPath);
      log(`${book.bookName} `);

      for (let chapter = 1; chapter <= book.chapters; chapter++) {
        const markdownFile = `${currentPath}/${chapter}.md`;
